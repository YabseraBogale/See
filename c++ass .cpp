...
int main(){
	...
	do{
		...
		l++;
		A=to_string(l);
		cout<<"your  file number is:"<<A<<endl;
		people = new Donare[num_people];
		file_in.open("file number "+A+".txt");
		file_in<<"Personal Information of Donares"<<endl;
		...
		for(short i=0;i<num_people;i++){
			...
			cin>>wieght;
			up:
			cout<<"Enter gender 1 for male"<<endl; 
			cout<<"Enter gender 2 for female: ";
			cin>>gender;
			if(gender=='1') {
				gender='M';
			}
		...
		file_out.open("file number "+A+".txt");
		while(file_out){
